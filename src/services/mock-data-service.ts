...
export const mockReviewSummaries: ProductReviewSummary[] =
  generateExtendedReviewSummaries(mockProducts, mockReviews);
...
export class MockDataService {
  ...
  static getProductReviewSummary(
    productId: number,
  ): ProductReviewSummary | undefined {
    return mockReviewSummaries.find(
  ...
  static getProductsByPriceRange(
    minPrice: number,
    maxPrice: number,
  ): Product[] {
    return mockProducts.filter(
  ...
  static getProductsByDiscount(minDiscount: number): Product[] {
    return mockProducts.filter(
      (product) => product.discount && product.discount >= minDiscount,
    );
  }
