...
export default function ProductPage({ params }: ProductPageProps) {
    ...
    const loadProductData = async () => {
      try {
        ...
        if (productResponse.data.success) {
          ...
          if (relatedResponse.data.success) {
            const related = relatedResponse.data.data
              .filter(
                (p) =>
                  p.category?.id === productResponse.data.data.category?.id &&
                  p.id !== productId,
              )
              .slice(0, 4);
        ...
        const summaryResponse =
          await reviewService.getProductReviewSummary(productId);
  ...
  const handleHelpful = async (reviewId: number, isHelpful: boolean) => {
    ...
    toast.success(
      isHelpful
        ? "Yardımcı olarak işaretlendi"
        : "Yardımcı olmadı olarak işaretlendi",
    );
  };
