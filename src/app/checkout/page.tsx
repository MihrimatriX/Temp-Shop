...
export default function CheckoutPage() {
  ...
  const backendType = useBackendStore((s) => s.config.type);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Real backends typically require auth + saved address/payment method IDs
      if (backendType !== "mock" && !token) {
        toast.error("Sipariş oluşturmak için giriş yapmalısınız");
        return;
      }

      // If user is authenticated (especially in mock backend), create order via OrderService
      if (token) {
        const orderService = new OrderService();
        const response = await orderService.createOrder(
          {
            // Mock backend uses the authenticated user's default address/method.
            shippingAddressId: 1,
            paymentMethodId: 1,
            items: items.map((ci) => ({
            ...
            notes: `Checkout: ${formData.firstName} ${formData.lastName}`,
          },
          token,
        );

        if (!response.data.success) {
          toast.error(response.data.message || "Sipariş oluşturulamadı");
          return;
        }
        ...
        router.push(
          `/order-success?orderNumber=${response.data.data.orderNumber}`,
        );
        return;
      }

      // Guest checkout (mock-only UX)
      await new Promise((resolve) => setTimeout(resolve, 1200));
      const guestOrderNumber = `ORD-${Date.now()}`;
      clearCart();
      toast.success("Siparişiniz başarıyla oluşturuldu!");
      router.push(`/order-success?orderNumber=${guestOrderNumber}`);
    } catch (error) {
