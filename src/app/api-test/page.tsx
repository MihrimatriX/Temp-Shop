          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                <Badge variant={backendType === 'mock' ? 'secondary' : 'default'}>
                  Backend Type: {backendType}
                </Badge>
                <span className="text-sm text-muted-foreground">
                  {backendType === 'mock' ? 'Mock data kullanılıyor' : 
                   backendType === 'dotnet' ? '.NET Core backend kullanılıyor' :
                   backendType === 'spring' ? 'Spring Boot backend kullanılıyor' : 'Bilinmeyen backend'}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <Button onClick={testCategories} disabled={loading} variant="outline">
                  {loading ? 'Yükleniyor...' : 'Kategorileri Test Et'}
                </Button>
                <Button onClick={testFeaturedProducts} disabled={loading} variant="outline">
                  {loading ? 'Yükleniyor...' : 'Öne Çıkan Ürünleri Test Et'}
                </Button>
                <Button onClick={testReviews} disabled={loading} variant="outline">
                  {loading ? 'Yükleniyor...' : 'Yorumları Test Et'}
                </Button>
                <Button onClick={testReviewSummary} disabled={loading} variant="outline">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {products.slice(0, 6).map((product) => (
                  <div key={product.id} className="border rounded-lg p-4">
                    <h3 className="font-semibold text-sm mb-2">{product.productName}</h3>
        ...
        {reviewSummary && (
          <Card className="mb-8">
            <CardHeader>
              ...
              <CardDescription>API'den gelen yorum özet verileri</CardDescription>
            </CardHeader>
            <CardContent>
