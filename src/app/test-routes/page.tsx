...
export default function TestRoutesPage() {
  const routes = [
    ...
    {
      path: "/categories/electronics/phones-tablets",
      name: "Telefon & Tablet",
    },
    ...
    // Ürün sayfaları
    {
      path: "/categories/home-furniture/pillows/organic-pillow",
      name: "Organik Yastık",
    },
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4">
        ...
        <p className="text-gray-600 mb-8">
          Aşağıdaki linklere tıklayarak route'ların çalışıp çalışmadığını test
          edebilirsiniz.
        </p>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {routes.map((route, index) => (
            <Link
              key={index}
              href={route.path}
              className="block p-4 bg-card rounded-lg shadow hover:shadow-md transition-shadow border border-border hover:border-primary/40"
            >
              <div className="font-medium text-primary hover:text-primary/80">
                {route.name}
              </div>
              <div className="text-sm text-muted-foreground mt-1">{route.path}</div>
            </Link>
          ))}
        </div>

        <div className="mt-8 p-4 bg-primary/10 border border-border rounded-lg">
          <h2 className="text-lg font-semibold mb-2">Test Edilecek URL'ler:</h2>
          <ul className="text-sm space-y-1">
            <li>
              •{" "}
              <code>
                http://localhost:3000/categories/home-furniture/pillows
              </code>
            </li>
            <li>
              •{" "}
              <code>
                http://localhost:3000/categories/home-furniture/home-textiles
              </code>
            </li>
            <li>
              •{" "}
              <code>
                http://localhost:3000/categories/home-furniture/kitchen
              </code>
            </li>
            <li>
              • <code>http://localhost:3000/categories/women/clothing</code>
            </li>
            <li>
              •{" "}
              <code>
                http://localhost:3000/categories/electronics/phones-tablets
              </code>
            </li>
            <li>
              •{" "}
              <code>
                http://localhost:3000/categories/home-furniture/pillows/organic-pillow
              </code>
            </li>
          </ul>
        </div>
